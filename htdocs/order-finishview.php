    ...
    for ($i = 1; $i <= $num; $i++) {
        ...
        while ($rowadd = $result_add->fetch_assoc()) {
            if ($rowadd['數量'] != 0) {
                ...
                $result_price2 = mysqli_query($conn, "SELECT menu_price AS price FROM menu Where menu_id='$fff'");
                $row_price2 = mysqli_fetch_array($result_price2);
                $total = $total + $row_price2['price'] * $rowadd['數量'];
                $smalltotal = $smalltotal + $row_price2['price'] * $rowadd['數量'];
            }
        }
